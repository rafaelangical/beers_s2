...
let BeerItem = ({ beerById, closeModal }) => (
  beerById ?
  <Container style={{minWidth: '100vw', minHeight: '100vh', top: '5%', left: 0, right: 0, bottom: 0, position: 'fixed', flexDirection: 'column'}}>
    <Modal.Dialog style={{width: '80%', alignContent: 'center', margin: '0 auto', justifyContent: 'center'}}>
      <Modal.Header>
        <Modal.Title style={{color: 'red'}}>{beerById.name}</Modal.Title>
      </Modal.Header>

      <Modal.Body style={{overflow:'hidden'}}>
        <img src={beerById.image_url} alt="cerveja" className="cerveja-image"/>
        <p>{beerById.description}</p>
        <strong>First breweb: <span>{beerById.first_brewed}</span></strong>
      </Modal.Body>
      <Modal.Footer>
...
const mapStateToProps = (state) => ({ beerById: state.beerById })

BeerItem = connect(mapStateToProps,null)(BeerItem)

const mapDispatchToProps = {
  closeModal: closeModal,
};

BeerItem = connect(null,mapDispatchToProps)(BeerItem);
